...
- **Align before big work.** Ambiguous or multi-path tasks: ask with numbered
  choices (recommend one), wait for pick. Loop until aligned. Trivial
  one-file fixes can skip.
- **Commit feature by feature.** One logical change per commit. Conventional
  Commits via caveman-commit rules: subject ≤50 chars, body only for
  non-obvious why. No AI attribution fluff.
- Stage only intended files; never commit secrets or real `config.json` paths
  (none exist — keep it that way). `AGENTS.md` is tracked intentionally;
  `.gitignore` already covers `.build/`, `*.app`, `.DS_Store`.
- Verify before committing: `swift build` clean, `--selftest` green, bundle
  builds via `./build.sh`, and any behavior change smoke-tested with the
